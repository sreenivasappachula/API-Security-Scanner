...
func TestLoginIsJWT(url string, paths []string) (string, User) {
...
func TestJWT(endpointURL string, paths []string, credentials User) ([]string, error) {
	...
	if resp.StatusCode == 200 {
		...
		foundIssues, err := JWTAlgoNone(url_org, paths, token, bearerType)
		fmt.Println("Testing JWT confusion Vulnerability...")
		foundconfustionIssues, err1 := JWTConfusionAttack(url_org, paths, token, bearerType)
...
func JWTConfusionAttack(url_org string, paths []string, token string, bearerType string) ([]string, error) {
...
func JWTAlgoNone(url_org string, paths []string, token string, bearerType string) ([]string, error) {
